...
#include "gma-minstrel-ht-wifi-manager.h"
...
NS_LOG_COMPONENT_DEFINE("GmaMinstrelHtWifiManager");
...
NS_OBJECT_ENSURE_REGISTERED(GmaMinstrelHtWifiManager);

TypeId
GmaMinstrelHtWifiManager::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::GmaMinstrelHtWifiManager")
            .SetParent<WifiRemoteStationManager>()
            .AddConstructor<GmaMinstrelHtWifiManager>()
            .SetGroupName("Wifi")
            .AddAttribute("UpdateStatistics",
                          "The interval between updating statistics table",
                          TimeValue(MilliSeconds(50)),
                          MakeTimeAccessor(&GmaMinstrelHtWifiManager::m_updateStats),
                          MakeTimeChecker())
            .AddAttribute("LegacyUpdateStatistics",
                          "The interval between updating statistics table (for legacy Minstrel)",
                          TimeValue(MilliSeconds(100)),
                          MakeTimeAccessor(&GmaMinstrelHtWifiManager::m_legacyUpdateStats),
                          MakeTimeChecker())
            .AddAttribute("LookAroundRate",
                          "The percentage to try other rates (for legacy Minstrel)",
                          UintegerValue(10),
                          MakeUintegerAccessor(&GmaMinstrelHtWifiManager::m_lookAroundRate),
                          MakeUintegerChecker<uint8_t>(0, 100))
            .AddAttribute("EWMA",
                          "EWMA level",
                          UintegerValue(75),
                          MakeUintegerAccessor(&GmaMinstrelHtWifiManager::m_ewmaLevel),
                          MakeUintegerChecker<uint8_t>(0, 100))
            .AddAttribute("SampleColumn",
                          "The number of columns used for sampling",
                          UintegerValue(10),
                          MakeUintegerAccessor(&GmaMinstrelHtWifiManager::m_nSampleCol),
                          MakeUintegerChecker<uint8_t>())
            .AddAttribute("PacketLength",
                          "The packet length used for calculating mode TxTime (bytes)",
                          UintegerValue(1200),
                          MakeUintegerAccessor(&GmaMinstrelHtWifiManager::m_frameLength),
                          MakeUintegerChecker<uint32_t>())
            .AddAttribute("UseLatestAmendmentOnly",
                          "Use only the latest amendment when it is supported by both peers",
                          BooleanValue(true),
                          MakeBooleanAccessor(&GmaMinstrelHtWifiManager::m_useLatestAmendmentOnly),
                          MakeBooleanChecker())
            .AddAttribute("PrintStats",
                          "Control the printing of the statistics table",
                          BooleanValue(false),
                          MakeBooleanAccessor(&GmaMinstrelHtWifiManager::m_printStats),
                          MakeBooleanChecker())
            .AddTraceSource("Rate",
                            "Traced value for rate changes (b/s)",
                            MakeTraceSourceAccessor(&GmaMinstrelHtWifiManager::m_currentRate),
                            "ns3::TracedValueCallback::Uint64")
            .AddAttribute("MeasurementInterval",
                          "Measurement Interval",
                          TimeValue(Seconds(1.0)),
                          MakeTimeAccessor(&GmaMinstrelHtWifiManager::m_measurementInterval),
                          MakeTimeChecker())
            .AddAttribute("MeasurementGuardInterval",
                          "Guard time between two measurement intervals",
                          TimeValue(Seconds(0)),
                          MakeTimeAccessor(&GmaMinstrelHtWifiManager::m_measurementGuardInterval),
                          MakeTimeChecker())
            .AddTraceSource("RateMeasurement",
                            "The transmission rate Measurement",
                            MakeTraceSourceAccessor(&GmaMinstrelHtWifiManager::m_rateMeasurement),
                            "ns3::GmaMinstrelHtWifiManager::RateMeasurementTracedCallback");
    return tid;
}

GmaMinstrelHtWifiManager::GmaMinstrelHtWifiManager()
    : m_numGroups(0),
      m_numRates(0),
      m_currentRate(0)
{
...
int64_t
GmaMinstrelHtWifiManager::AssignStreams(int64_t stream)
{
...
void
GmaMinstrelHtWifiManager::SetupPhy(const Ptr<WifiPhy> phy)
{
...
void
GmaMinstrelHtWifiManager::SetupMac(const Ptr<WifiMac> mac)
{
...
void
GmaMinstrelHtWifiManager::DoInitialize()
{
    NS_LOG_FUNCTION(this);
    /**
     ...
     * station.
     */
    Simulator::Schedule(m_measurementGuardInterval, &GmaMinstrelHtWifiManager::MeasurementGuardIntervalEnd, this);
...
bool
GmaMinstrelHtWifiManager::IsValidMcs(Ptr<WifiPhy> phy,
                                  uint8_t streams,
                                  uint16_t chWidth,
                                  WifiMode mode)
{
...
Time
GmaMinstrelHtWifiManager::CalculateMpduTxDuration(Ptr<WifiPhy> phy,
                                               uint8_t streams,
                                               uint16_t gi,
                                               uint16_t chWidth,
                                               WifiMode mode,
...
Time
GmaMinstrelHtWifiManager::GetFirstMpduTxTime(uint8_t groupId, WifiMode mode) const
{
...
void
GmaMinstrelHtWifiManager::AddFirstMpduTxTime(uint8_t groupId, WifiMode mode, Time t)
{
...
Time
GmaMinstrelHtWifiManager::GetMpduTxTime(uint8_t groupId, WifiMode mode) const
{
    NS_LOG_FUNCTION(this << +groupId << mode);
...
void
GmaMinstrelHtWifiManager::AddMpduTxTime(uint8_t groupId, WifiMode mode, Time t)
{
    NS_LOG_FUNCTION(this << +groupId << mode << t);
...
WifiRemoteStation*
GmaMinstrelHtWifiManager::DoCreateStation() const
{
    NS_LOG_FUNCTION(this);
...
void
GmaMinstrelHtWifiManager::CheckInit(MinstrelHtWifiRemoteStation* station)
{
...
void
GmaMinstrelHtWifiManager::DoReportRxOk(WifiRemoteStation* st, double rxSnr, WifiMode txMode)
{
...
void
GmaMinstrelHtWifiManager::DoReportRtsFailed(WifiRemoteStation* st)
{
    NS_LOG_FUNCTION(this << st);
...
void
GmaMinstrelHtWifiManager::DoReportRtsOk(WifiRemoteStation* st,
                                     double ctsSnr,
                                     WifiMode ctsMode,
                                     double rtsSnr)
{
...
void
GmaMinstrelHtWifiManager::DoReportFinalRtsFailed(WifiRemoteStation* st)
{
    NS_LOG_FUNCTION(this << st);
    auto station = static_cast<MinstrelHtWifiRemoteStation*>(st);
...
void
GmaMinstrelHtWifiManager::DoReportDataFailed(WifiRemoteStation* st)
{
    NS_LOG_FUNCTION(this << st);
    auto station = static_cast<MinstrelHtWifiRemoteStation*>(st);
...
void
GmaMinstrelHtWifiManager::DoReportDataOk(WifiRemoteStation* st,
                                      double ackSnr,
                                      WifiMode ackMode,
                                      double dataSnr,
...
void
GmaMinstrelHtWifiManager::DoReportFinalDataFailed(WifiRemoteStation* st)
{
    NS_LOG_FUNCTION(this << st);
    auto station = static_cast<MinstrelHtWifiRemoteStation*>(st);
...
void
GmaMinstrelHtWifiManager::DoReportAmpduTxStatus(WifiRemoteStation* st,
...
void
GmaMinstrelHtWifiManager::UpdateRate(MinstrelHtWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
void
GmaMinstrelHtWifiManager::UpdateRetry(MinstrelHtWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
    station->m_shortRetry = 0;
    station->m_longRetry = 0;
...
void
GmaMinstrelHtWifiManager::UpdatePacketCounters(MinstrelHtWifiRemoteStation* station,
                                            uint16_t nSuccessfulMpdus,
                                            uint16_t nFailedMpdus)
{
...
uint16_t
GmaMinstrelHtWifiManager::UpdateRateAfterAllowedWidth(uint16_t txRate, uint16_t allowedWidth)
{
...
WifiTxVector
GmaMinstrelHtWifiManager::DoGetDataTxVector(WifiRemoteStation* st, uint16_t allowedWidth)
{
    ...
    station->m_lastChannelWidth = allowedWidth;
...
WifiTxVector
GmaMinstrelHtWifiManager::DoGetRtsTxVector(WifiRemoteStation* st)
{
    NS_LOG_FUNCTION(this << st);
    auto station = static_cast<MinstrelHtWifiRemoteStation*>(st);
...
bool
GmaMinstrelHtWifiManager::DoNeedRetransmission(WifiRemoteStation* st,
...
uint32_t
GmaMinstrelHtWifiManager::CountRetries(MinstrelHtWifiRemoteStation* station)
{
    uint8_t maxProbRateId = GetRateId(station->m_maxProbRate);
    uint8_t maxProbGroupId = GetGroupId(station->m_maxProbRate);
    uint8_t maxTpRateId = GetRateId(station->m_maxTpRate);
...
uint16_t
GmaMinstrelHtWifiManager::GetNextSample(MinstrelHtWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
void
GmaMinstrelHtWifiManager::SetNextSample(MinstrelHtWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
    do
    {
...
uint16_t
GmaMinstrelHtWifiManager::FindRate(MinstrelHtWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
void
GmaMinstrelHtWifiManager::UpdateStats(MinstrelHtWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
double
GmaMinstrelHtWifiManager::CalculateThroughput(MinstrelHtWifiRemoteStation* station,
                                           uint8_t groupId,
                                           uint8_t rateId,
                                           double ewmaProb)
{
...
void
GmaMinstrelHtWifiManager::SetBestProbabilityRate(MinstrelHtWifiRemoteStation* station, uint16_t index)
{
    GroupInfo* group;
 ...
 */
void
GmaMinstrelHtWifiManager::SetBestStationThRates(MinstrelHtWifiRemoteStation* station, uint16_t index)
{
    uint8_t groupId;
    uint8_t rateId;
    double th;
...
void
GmaMinstrelHtWifiManager::RateInit(MinstrelHtWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
void
GmaMinstrelHtWifiManager::CalculateRetransmits(MinstrelHtWifiRemoteStation* station, uint16_t index)
{
...
void
GmaMinstrelHtWifiManager::CalculateRetransmits(MinstrelHtWifiRemoteStation* station,
                                            uint8_t groupId,
                                            uint8_t rateId)
{
...
double
GmaMinstrelHtWifiManager::CalculateEwmsd(double oldEwmsd,
                                      double currentProb,
                                      double ewmaProb,
                                      double weight)
{
...
void
GmaMinstrelHtWifiManager::InitSampleTable(MinstrelHtWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
void
GmaMinstrelHtWifiManager::PrintTable(MinstrelHtWifiRemoteStation* station)
{
...
void
GmaMinstrelHtWifiManager::StatsDump(MinstrelHtWifiRemoteStation* station,
                                 uint8_t groupId,
                                 std::ofstream& of)
{
...
uint16_t
GmaMinstrelHtWifiManager::GetIndex(uint8_t groupId, uint8_t rateId)
{
...
uint8_t
GmaMinstrelHtWifiManager::GetRateId(uint16_t index)
{
...
uint8_t
GmaMinstrelHtWifiManager::GetGroupId(uint16_t index)
{
    NS_LOG_FUNCTION(this << index);
...
uint8_t
GmaMinstrelHtWifiManager::GetHtGroupId(uint8_t txstreams, uint16_t gi, uint16_t chWidth)
{
...
uint8_t
GmaMinstrelHtWifiManager::GetVhtGroupId(uint8_t txstreams, uint16_t gi, uint16_t chWidth)
{
    NS_LOG_FUNCTION(this << +txstreams << gi << chWidth);
    uint8_t giIndex = (gi == 400) ? 1 : 0;
    uint8_t widthIndex;
...
uint8_t
GmaMinstrelHtWifiManager::GetHeGroupId(uint8_t txstreams, uint16_t gi, uint16_t chWidth)
{
    NS_LOG_FUNCTION(this << +txstreams << gi << chWidth);
    uint8_t giIndex;
    if (gi == 800)
...
uint16_t
GmaMinstrelHtWifiManager::GetLowestIndex(MinstrelHtWifiRemoteStation* station)
{
    NS_LOG_FUNCTION(this << station);
...
uint16_t
GmaMinstrelHtWifiManager::GetLowestIndex(MinstrelHtWifiRemoteStation* station, uint8_t groupId)
{
...
WifiModeList
GmaMinstrelHtWifiManager::GetHeDeviceMcsList() const
{
...
WifiModeList
GmaMinstrelHtWifiManager::GetVhtDeviceMcsList() const
{
...
WifiModeList
GmaMinstrelHtWifiManager::GetHtDeviceMcsList() const
{
...
void
GmaMinstrelHtWifiManager::MeasurementGuardIntervalEnd()
{
    Simulator::Schedule(m_measurementInterval, &GmaMinstrelHtWifiManager::MeasurementIntervalEnd, this);
    m_measurementActive = true; //start measurement
}

void
GmaMinstrelHtWifiManager::MeasurementIntervalEnd()
{
    //std::cout <<"Measurement Interval end:" << Now().GetSeconds() << " map size:" << m_addrToStationMap.size() << std::endl;
    auto copMap = m_addrToStationMap; //copy the map, since it may be overwite in the DoGetDataTxVector function!

    std::map<Mac48Address, MinstrelHtWifiRemoteStation*>::iterator iter = copMap.begin();
    while(iter!=copMap.end())
    {
        //if(iter->second->m_lastChannelWidth != 0)
        {
            WifiTxVector txVector = DoGetDataTxVector(iter->second, GetChannelWidth(iter->second));
            /*std::cout << Simulator::Now().GetSeconds()
            << " ADDR:" << iter->first
            << " tx_rate:" << txVector.GetMode().GetDataRate(txVector)
            << " mode:" << txVector.GetMode().GetUniqueName()
            << " BW:" << iter->second->m_lastChannelWidth
            << std::endl;*/
