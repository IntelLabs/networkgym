...
TcpBbr::UpdateBtlBw(Ptr<TcpSocketState> tcb, const TcpRateOps::TcpRateSample& rs)
{
    ...
    if (rs.m_deliveryRate == 0 || !rs.IsValid())
    {
        return;
    }
    ...
    fileName <<"bbr-"<<this<<".csv";
    std::ofstream myfile;
    myfile.open (fileName.str ().c_str (), std::ios::out | std::ios::app);
    myfile << Simulator::Now ().GetSeconds () << ", ";
    myfile << "state: " << BbrModeName[m_state] << ", ";
    myfile << "m_maxBwFilter.GetBest(): " << m_maxBwFilter.GetBest() << ", ";
    myfile << "m_pacingGain: " << m_pacingGain << ", ";
    myfile << "m_rtProp: " << m_rtProp << ", ";
    myfile << "m_priorCwnd: " << m_priorCwnd << ", ";
    myfile << "tcb->m_cWnd.Get(): " << tcb->m_cWnd.Get() << ", ";
    myfile << "rs.m_deliveryRate: " << rs.m_deliveryRate << ", ";
    myfile << "rs.IsValid : " <<rs.IsValid() << ", ";
    myfile << "rs.m_interval:" << rs.m_interval.GetSeconds() << ", ";
    myfile << "rs.m_delivered : " <<rs.m_delivered << ", ";
    myfile << "rs.m_priorDelivered : " <<rs.m_priorDelivered << ", ";
    myfile << "rs.m_priorTime : " <<rs.m_priorTime.GetSeconds() << ", ";
    myfile << "rs.m_sendElapsed : " <<rs.m_sendElapsed.GetSeconds() << ", ";
    myfile << "rs.m_ackElapsed : " <<rs.m_ackElapsed.GetSeconds() << ", ";
    myfile << "rs.m_bytesLoss : " <<rs.m_bytesLoss << ", ";
    myfile << "rs.m_priorInFlight : " <<rs.m_priorInFlight << ", ";
    myfile << "rs.m_ackedSacked : " <<rs.m_ackedSacked << ", ";
    myfile << "rs.m_isAppLimited: " << rs.m_isAppLimited << ", ";
    myfile << std::endl;
    myfile.close();*/
